...
exports.getAuthTokens = (user) => {
  ...
  const authToken = jwt.sign({user: userObject}, process.env.AUTH_TOKEN_SECRET, jwtAlgorithm("10m"));
  const refreshToken = jwt.sign({user: userObject}, process.env.REFRESH_TOKEN_SECRET, jwtAlgorithm("1d"));
